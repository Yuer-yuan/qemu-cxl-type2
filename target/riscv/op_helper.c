...
void helper_memory_fence(CPURISCVState *env)
{
    if (memory_region_persist_pending() != MEMTX_OK) {
        /*
         * FENCE has no architectural I/O-error return. A failed model
         * persistence handoff therefore invalidates the emulation run rather
         * than inventing a guest-visible exception or reporting durability.
         */
        cpu_abort(env_cpu(env),
                  "CXL persistence-domain handoff failed after FENCE");
    }
}
